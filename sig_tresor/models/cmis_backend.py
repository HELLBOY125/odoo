...
class CmisBackend(models.Model):
    ...
    def check_directory_of_write_new(self, path, fname, fcontent, ftype, fencoding):
        """Check access right to write from the path"""
        datas_fname = fname #'testdoc'
        res = ""
        for this in self:
            # login with the cmis account
            folder_path_write = this.initial_directory_write
            folder_path_write_ = this.initial_directory_write + 'TRESOR/' + path
            print("chemin %s " % folder_path_write_)
            path_write_objectid = self.get_folder_by_path(
                folder_path_write_,
            ...
            if path_write_objectid:
                try:
                    # path_write_objectid.createDocumentFromString(
                    #     datas_fname,
                    #     contentString=fcontent,
                    #     contentType=ftype)
                    # someDoc =
                    path_write_objectid.createDocument(
                        datas_fname,
                        contentFile=fcontent,
                        contentType=ftype,
                        # contentEncoding=fencoding
                    )
                except cmislib.exceptions.UpdateConflictException:
                    _logger.exception("The test file already exists in the DMS \n. Please remove it and try again.")
                    # raise CMISError(
                    #     _("The test file already exists in the DMS. "
                    #       "Please remove it and try again."))
                except cmislib.exceptions.RuntimeException:
                    ...
                    #     ("Please check your access right."))
            if path_write_objectid is not False:
                _logger.info("Path is correct for : %s" % path_write_objectid)
                # raise UserError(_("Path is correct for : %s") %
                #                 path_write_objectid)
                res = path_write_objectid
                print("path_write_objectid %s " % path_write_objectid)

                # props = someDoc.getProperties()
                # for k, v in props.items():
                #     print('%s:%s' % (k, v))
            else:
                _logger.error("Error path for : %s" % path_write_objectid)
                # raise CMISError(_("Error path for : %s") %
                #                 path_write_objectid)
        return res

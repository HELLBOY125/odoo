...
class AccountAssetAsset(models.Model):
    ...
    code_barre = fields.Char(string='Code à barre',readonly=True, states={'draft': [('readonly', False)]})
    marque =  fields.Char(string="Marque",readonly=True, states={'draft': [('readonly', False)]})
    num_serie = fields.Char(string="N° de série",readonly=True, states={'draft': [('readonly', False)]})
    ...
    date_acquisition = fields.Date(string='Date', required=True, readonly=True, states={'draft': [('readonly', False)]}, default=fields.Date.context_today)
    ...
    method_period = fields.Integer(string='Number of Months in a Period',   
        help="The amount of time between two depreciations, in months",compute="_get_number_period")
    method_period_sel = fields.Selection([('year', 'Année'), ('month', 'Mois')], string='Une entrée chaque', required=True, default='year',readonly=True, states={'draft': [('readonly', False)]})
    ...
    invoice_line_id = fields.Many2one('account.move.line', string="Ligne de Bon d'entrée Valeur", copy=False,readonly=True, states={'draft': [('readonly', False)]})
     # Adapt for import fields
    already_depreciated_amount_import = fields.Monetary(help="In case of an import from another software, you might need to use this field to have the right depreciation table report. This is the value that was already depreciated with entries not computed from this model",readonly=True, states={'draft': [('readonly', False)]})
    depreciation_number_import = fields.Integer(help="In case of an import from another software, provide the number of depreciations already done before starting with Odoo.",readonly=True, states={'draft': [('readonly', False)]})
    first_depreciation_date_import = fields.Date(help="In case of an import from another software, provide the first depreciation date in it.",readonly=True, states={'draft': [('readonly', False)]})
    ...
    def _get_number_period(self):
        self.ensure_one()
        if self.method_period_sel =='year':
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        domain = []
        if name:
            domain = ['|','|',('code_barre', '=ilike', name.split(' ')[0] + '%'),('code', '=ilike', name.split(' ')[0] + '%'), ('name', operator, name)]
            if operator in expression.NEGATIVE_TERM_OPERATORS:
                domain = ['&', '!'] + domain[1:]
        local_ids = self._search(expression.AND([domain, args]), limit=limit, access_rights_uid=name_get_uid)
        return models.lazy_name_get(self.browse(local_ids).with_user(name_get_uid))
     
    
    
        
    @api.depends('value', 'salvage_value', 'depreciation_line_ids.move_check', 'depreciation_line_ids.amount','already_depreciated_amount_import')
    def _amount_residual(self):
        for rec in self:
            ...
            print ('total_amount____',total_amount)         
            rec.value_residual = rec.value -  rec.salvage_value  - rec.already_depreciated_amount_import- total_amount
    ...
    def _compute_board_amount(self, sequence, residual_amount, amount_to_depr, undone_dotation_number, posted_depreciation_line_ids, total_days, depreciation_date):
        amount = 0
        print ('residual_amount_________',residual_amount)
    ...
    def compute_depreciation_board(self):
        ...
        print('self.value_residual________',self.value_residual)
        if self.value_residual != 0.0:
            ...
            for x in range(len(posted_depreciation_line_ids), undone_dotation_number):
                ...
                vals = {
                    ...
                    'depreciated_value': self.value - (self.salvage_value + residual_amount+self.already_depreciated_amount_import),
    ...
    def post_move(self,reason):
        for rec in self:
            move_ids = rec._get_disposal_moves()
            if not move_ids:
                for depreciation_line in rec.depreciation_line_ids:
                    if depreciation_line.move_id:
                        move_ids.append(depreciation_line.move_id.id)
            for move in move_ids:           
                move_id=self.env['account.move'].browse(move)
                move_id.write({'ref':'%s- (%s)' %  (rec.name,reason)})
                move_id.action_post()
...
class AccountAssetDepreciationLine(models.Model):
    ...
    value = fields.Float(related='asset_id.value',string="Valeur d'origine",digits=0,readonly=True)
    date = fields.Date(related='asset_id.date',string='Date de mise en service', readonly=True)
    date_acquisition = fields.Date(related='asset_id.date_acquisition',string="Date d'acquisition",readonly=True)
    ...
    depreciated_value = fields.Float(string='Cumulative Depreciation',digits=0, required=True)

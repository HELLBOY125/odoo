...
#         if self.holiday_type not in ['employee','company']:
#             self.accrual = False
    ...
    def name_get(self):
        ...
        for allocation in self:
            allocation_leave = allocation.holiday_status_id.sudo().name or ''
...
#         if allocation_type=='accrual':
    ...
    def _update_accrual(self):
        ...
        allocations = self.search([
            ...
            ('state', '=', 'validate'),
            ('holiday_type', '=', 'employee'),
            ('holiday_status_id','=',self.env.ref('spc_leaves.holiday_status_outing_authorization').id)
        ])
    ...
    def _update_accrual_allocation(self):
        ...
        if not self.allocation_type:  # pragma: no cover
    ...
    def _check_dates_debut_cumul_fin(self):
        if self.date_from and self.date_to:

                if  self.date_to <= self.date_from:
            
                        raise ValidationError(_('La date de début de cumul doit être antérieure à la date de fin'))
...
#      
#             self.date_from = self.employee_id.sudo().service_start_date
...
#         elif self.accrual_method == 'prorate':
#             values_view = workable_days.values()
#             value_iterator = iter(values_view)
#             first_value = next(value_iterator)
#             print(first_value['days'])
#             return HrLeaveAllocationAccruementEntry(
#                 days_accrued=days_to_accrue * (days_worked / first_value['days']),
#                 accrued_on=period_end.date(),

...
class OrgChartDepartment(models.Model):
    ...
    def get_children(self, emp, style=False):
        data = []
        company = self.env.user.company_id
        emp_data = {'id': emp.id, 'name': emp.name + "<br> <p style='color:#A9A9A9';>" + self._get_position(emp) + "</p>",
                    'title': self._get_department(emp),
                    'office': self._get_image(emp, style)}
        ...
        for child in childrens:
            ...
            if not sub_child:
                data.append({'id': child.id, 'name': child.name + "<br> <p style='color:#A9A9A9';>" + self._get_position(child) + "</p>",
                             'title': self._get_department(child),
                             'className': next_style,
                             'office': self._get_image(child, style)})
            else:
    ...
    def _get_department(self, emp):
        if emp.sudo().department_id:
            return emp.sudo().department_id.name
        return ""

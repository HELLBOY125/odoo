...
class AccountInvoice(models.Model):
    ...
    def open_asset_view(self):
        asset_ids=[]
        asset_ids=self.env['account.asset.asset'].sudo().search([('invoice_id', '=', self.id)]).ids
    ...
    def action_post(self):
        ...
        for inv in self:
            ...
            for mv_line in inv.invoice_line_ids:
                if mv_line.asset_category_id.multiple_assets_per_line:
                    for line in range(int(mv_line.quantity)):
...
class AccountInvoiceLine(models.Model):
    ...
    def asset_create(self):
        ...
        if self.asset_category_id:
            if self.asset_category_id.multiple_assets_per_line:
            ...
            sequence=len(self.env['account.asset.asset'].search([('category_id','=', self.asset_category_id.id)]))    
            if sequence==0:
                sequence=sequence+1
            date= fields.date.today()
            print ('date_________',date)
            first_depreciation_manual_date="%s-12-31" %date.year
            vals = {
                'name': self.name,
                ...
                'code_barre': '%s%s' %(self.product_id.default_code,
                                       sequence),
    ...
    def _onchange_product_id(self):
        print ('_onchange_product_id______',self.move_id.type)
    ...
    def _set_additional_fields(self, invoice):
        print ('_set_additional_fields_____________________')
